...
const generateSoulboundHandlers = ({ abi, events, contractName }) =>
  `
  import { BigInt } from '@graphprotocol/graph-ts'
  import { ${contractName}, ${events.map(event => event._alias)} }
    from '../generated/${contractName}/${contractName}'
  import { BaseMetric, TransactionMetadata, InMemoryIncrementStore, EventType, MetricParam, BadgeType, BadgeMetricLookup, UserBadge } from '../generated/schema'
  ...
  ${events
    ...
    export function handle${event._alias}(event: ${event._alias}): void {
      // Entities can be loaded from the store using a string ID; this ID
      // needs to be unique across all entities of the same type
      let entity = BaseMetric.load(event.transaction.from.toHex())

      // Entities only exist after they have been saved to the store;
      // \`null\` checks allow to create entities on demand
      if (!entity) {
        entity = new BaseMetric(event.transaction.from.toHex())
