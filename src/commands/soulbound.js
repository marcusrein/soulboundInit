...
const { generateScaffold, writeScaffold, generateSoulboundScaffold } = require('../command-helpers/scaffold')
...
const HELP = `
${chalk.bold('graph soulbound')} [options] [subgraph-name] [directory]
...
module.exports = {
  description: 'Creates a subgraph with BaseMetrics for use in the soulbound studio',
  options: {
  ...
  run: async toolbox => {
    ...
    // NOTE: We are going to force the user to go through the init form
...
const initSubgraphFromContract = async (
  ...
  let scaffold = await withSpinner(
    ...
    async spinner => {
      let scaffold = await generateSoulboundScaffold(
        {
          protocolInstance,
          subgraphName,
          abi,
